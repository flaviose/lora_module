...
##############################################################################
-include extensions/common/makefiles/app_options.mk
...
ifeq ($(BSP),l4)
-include extensions/common/makefiles/board_L451.mk
BOARD_TARGET=l4
endif

ifeq ($(BSP),l0)
-include extensions/common/makefiles/board_L071.mk
BOARD_TARGET=l0
endif
...
#-----------------------------------------------------------------------------
BUILD_TARGET = $(APP_NAME)_$(TARGET)
...
GIT_HASH_INT := 0x$(shell git rev-parse --short=8 HEAD)
...
#-----------------------------------------------------------------------------
APP_C_SOURCES += \
	extensions/common/settings/settings.c \
	extensions/common/sha256/sha256.c \
	extensions/common/timer/timer.c \
	extensions/common/timer/timer_rtc.c

COMMON_C_INCLUDES += \
	-Iextensions/common/settings/ \
	-Iextensions/common/sha256/ \
	-Iextensions/common/timer/ \
	-Iextensions/common/
...
RADIO_HAL_C_SOURCES += \
	extensions/common/radio_hal/radio_utilities.c

COMMON_C_INCLUDES +=  \
	-I.\
	-Iextensions/common/radio_hal\
...
LBM_FLAGS += -DMODEM_HAL_DBG_TRACE_COLOR=0

.PHONY: $(BASIC_MODEM_LIB)
$(BASIC_MODEM_LIB):
	$(MAKE) -C $(LORA_BASICS_MODEM) basic_modem RADIO=$(TARGET_RADIO) MODEM_TRACE=$(LBM_TRACE) MCU_FLAGS="$(MCU_FLAGS)"  EXTRAFLAGS="$(LBM_FLAGS)" OPT="$(MODEM_OPT)" CRYPTO=$(CRYPTO) NB_OF_STACK=$(LBM_NB_OF_STACK) $(LBM_BUILD_OPTIONS) VERBOSE=$(VERBOSE) REGION=$(REGION) $(MTHREAD_FLAG)
