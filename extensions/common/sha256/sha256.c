...
last:
                ...
                for( int i = 0; i < 8; i++ ) {
                    uint32_t temp = ENDIAN_n2b32( state[i] );
                    hash[i * 4] = temp >> 0;
                    hash[i * 4 + 1] = temp >> 8;
                    hash[i * 4 + 2] = temp >> 16;
                    hash[i * 4 + 3] = temp >> 24;
...
static uint32_t state_i[8];
static uint8_t hash_i[32];
static uint32_t length_i;
static bool hash_set;

void sha256i_init( void ) {
    state_i[0] = 0x6a09e667;
    state_i[1] = 0xbb67ae85;
    state_i[2] = 0x3c6ef372;
    state_i[3] = 0xa54ff53a;
    state_i[4] = 0x510e527f;
    state_i[5] = 0x9b05688c;
    state_i[6] = 0x1f83d9ab;
    state_i[7] = 0x5be0cd19;
...
last:
            tmp.words[15] = ENDIAN_n2b32( bitlen );
            sha256_do( state_i, tmp.bytes );

            for( int i = 0; i < 8; i++ ) {
                uint32_t temp = ENDIAN_n2b32( state_i[i] );
                hash_i[i * 4] = temp >> 0;
                hash_i[i * 4 + 1] = temp >> 8;
                hash_i[i * 4 + 2] = temp >> 16;
                hash_i[i * 4 + 3] = temp >> 24;
            }
...
void sha256i_digest( uint8_t* hash ) {
    if( !hash_set ) {
        sha256i_update( NULL, 0 );
    }

    memcpy( hash, hash_i, 32 );
}
